...
import org.zwobble.hod.compiler.nullableToList
...
private fun generateTargetAssignment(
    ...
    val (pythonTarget, statements) = generateTargetCode(hodTarget, context)
...
internal fun generateTargetCode(
    ...
    return when (hodTarget) {
        is TargetNode.Variable ->
            Pair(
                PythonVariableReferenceNode(
                    name = context.name(hodTarget),
                    source = source
                ),
                listOf()
            )
        is TargetNode.Tuple -> {
            val (tupleMembers, statements) = hodTarget.elements.map { element ->
        ...
        is TargetNode.Fields -> {
            ...
            val statements = hodTarget.fields.flatMap { (fieldName, fieldTarget) ->
                generateTargetAssignment(
                    fieldTarget,
