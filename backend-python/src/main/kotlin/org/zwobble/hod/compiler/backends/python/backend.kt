...
import org.zwobble.hod.compiler.backends.python.ast.PythonAssignmentNode
import org.zwobble.hod.compiler.backends.python.ast.PythonStringLiteralNode
import org.zwobble.hod.compiler.backends.python.ast.PythonVariableReferenceNode
...
val topLevelPythonPackageName = "hod"
...
val backend = object: Backend {
    override fun compile(moduleSet: ModuleSet, target: Path) {
        for (module in moduleSet.modules) {
            when (module) {
                ...
                is Module.Native -> {
                    moduleWriter(target, hodModuleNameToPythonModuleName(module.name, moduleSet)).use { writer ->
    ...
    private fun nativeModuleSource(module: Module.Native): InputStream {
        val resourcePrefix = "org/zwobble/hod/compiler/backends/python/modules/"
...
private fun compileModule(module: Module.Hod, moduleSet: ModuleSet): PythonModule {
    val generateCode = generateCode(
        module = module,
        moduleSet = moduleSet
    )
    val builtins = """
        from hod.builtins import (
    ...
    val main = if (module.hasMain()) {
        // TODO: avoid _hod_main collision
        """
            def _hod_main():
                import sys as sys
                exit_code = main()
    ...
    return PythonModule(
        name = hodModuleNameToPythonModuleName(module.name, moduleSet),
        source = contents + main
    )
}

internal fun hodModuleNameToPythonModuleName(moduleName: List<Identifier>, moduleSet: ModuleSet): List<String> {
...
private fun builtinModule(): PythonModule {
    val contents = readResourceText("org/zwobble/hod/compiler/backends/python/modules/builtins.py")
    return PythonModule(
        name = listOf("hod", "builtins"),
        source = contents
    )
}
