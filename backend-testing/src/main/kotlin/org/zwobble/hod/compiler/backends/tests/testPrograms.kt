package org.zwobble.hod.compiler.backends.tests
...
sealed class TestProgramSource {
    ...
    class File(
        ...
        override val mainModule: List<Identifier>
            get() = listOf(Identifier(path.fileName.toString().removeSuffix(".hod")))
    }
...
fun testPrograms(): List<TestProgram> {
    return findTestFiles().map(fun(source): TestProgram {
        val mainPath = when (source) {
            ...
            is TestProgramSource.Directory -> source.path.resolve("src").resolve(source.mainModule.map(Identifier::value).joinToString("/") + ".hod")
        }
