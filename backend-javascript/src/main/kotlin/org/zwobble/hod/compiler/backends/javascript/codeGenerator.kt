...
import org.zwobble.hod.compiler.backends.ModuleCodeInspector
import org.zwobble.hod.compiler.backends.isConstant
...
import org.zwobble.hod.compiler.types.Symbol

internal fun generateCode(module: Module.Hod): JavascriptModuleNode {
...
private fun generateCode(module: Module.Hod, import: ImportNode): JavascriptStatementNode {
...
private fun generateCastImports(module: Module.Hod, context: CodeGenerationContext): List<JavascriptStatementNode> {
...
private fun generateImportExpression(importPath: ImportPath, module: Module.Hod, source: NodeSource): JavascriptFunctionCallNode {
...
private fun generateCodeForVarargsDeclaration(node: VarargsDeclarationNode, context: CodeGenerationContext): JavascriptStatementNode {
    ...
    return javascriptConst(
        name = generateName(node.name),
        expression = JavascriptFunctionCallNode(
            function = JavascriptVariableReferenceNode("\$hod.varargs", source = source),
            arguments = listOf(
...
private fun generateCodeForTarget(
    hodTarget: TargetNode
): JavascriptTargetNode {
    val source = NodeSource(hodTarget)
    return when (hodTarget) {
        is TargetNode.Variable -> {
            JavascriptVariableReferenceNode(generateName(hodTarget.name), source = source)
        }
        is TargetNode.Tuple -> JavascriptArrayDestructuringNode(
            elements = hodTarget.elements.map { targetElement ->
                generateCodeForTarget(targetElement)
            },
            source = source
        )
        is TargetNode.Fields -> JavascriptObjectDestructuringNode(
            properties = hodTarget.fields.map { (fieldName, fieldTarget) ->
